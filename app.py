...
from config import MAX_FILE_SIZE, IMAGE_EXTENSIONS, VIDEO_EXTENSIONS, MIME_TO_EXT, DOCUMENT_EXTENSIONS, AUTH_TOKEN
...
def token_required(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        if not token or token != AUTH_TOKEN:
            logger.warning(f"Token认证失败，来源IP: {request.remote_addr}")
            return jsonify({
                'status': 'error',
